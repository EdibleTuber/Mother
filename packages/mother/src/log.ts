...
// Usage summary — supports local models (zero cost)
export function logUsageSummary(
	...
	if (isLocalModel) {
		lines.push("**Cost: Local model (no cost)**");
	} else {
		lines.push(
	...
	if (isLocalModel) {
		console.log(
			chalk.dim(
				`           ${usage.input.toLocaleString()} in + ${usage.output.toLocaleString()} out (local model, no cost)`,
			),
		);
	} else {
		console.log(
...
export function logStartup(workingDir: string, sandbox: string): void {
	console.log("Starting mother bot...");
	console.log(`  Working directory: ${workingDir}`);
	console.log(`  Sandbox: ${sandbox}`);
}

export function logConnected(): void {
	console.log("Mother bot connected and listening!");
	console.log("");
}

export function logDisconnected(): void {
	console.log("Mother bot disconnected.");
}
