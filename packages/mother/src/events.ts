...
export type MotherEvent = ImmediateEvent | OneShotEvent | PeriodicEvent;
...
export class EventsWatcher {
	...
	/**
	 * Start watching for events. Call this after DiscordBot is ready.
	 */
	start(): void {
	...
	private parseEvent(content: string, filename: string): MotherEvent | null {
	...
	private execute(filename: string, event: MotherEvent, deleteAfter: boolean = true): void {
		// Format the message
		...
		// Create synthetic DiscordEvent
		const syntheticEvent: DiscordEvent = {
			type: "mention",
